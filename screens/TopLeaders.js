...
const callApi=async(uid,type)=>{
        ...
        let url5=global.BASE_URL+"css_mob/user_list_weekly.aspx?uid="+uid+'&type='+type;
      console.log(url5)
      fetch(url5)
      .then(item => item.json())
      .then(Vdta => {          
...
if(Data.length==0){
  return(
    ...
    <View style={{justifyContent: 'center',alignItems: 'center',height:'100%',
        backgroundColor:colors.background}} >
          <Text style={styles.sheading}>No Achievers list available at present...
          </Text>
        </View>
  )
}
        ...
        <ImageBackground source={require('../assets/botz/to-bg.png')} resizeMode={'stretch'} style={[styles.header,{flex:1.7}]}>
            <View style={{flexDirection:'row',justifyContent:'space-around'}}>
                  ...
                  <View  style={{felx:1,width:'100%',paddingVertical:5}}>
                        <Text style={styles.heading}>Rank Achievers</Text>
                  </View>
...
{/*  */}
<View  style={[styles.header,{flex:1.5,marginTop:25}]}>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-around',marginVertical:25}}>
                ...
                <Text style={[styles.sheading,{fontSize:40}]}>2</Text>
                    {/* <Image source={require('../assets/botz/arrw1.png')} resizeMode={'contain'} style={{alignSelf:'center',marginVertical:5}}/> */}
                    <ImageBackground source={require('../assets/botz/no2.png')} resizeMode={'stretch'} style={{width:100,alignItems: 'center',height:160,paddingTop:45}}>
                      <Image source={require('../assets/logom.png')} resizeMode={'contain'} style={{width:60,height:60}} />
                    </ImageBackground>
                    <Text style={[styles.text,{textAlign:'center',color:'#fff',fontWeight: 'bold',fontSize:15}]}>{Data[1].name}</Text>                    
                </View>
                <View style={{flexDirection:'column',justifyContent:'center',alignSelf: 'center',marginHorizontal:-25,zIndex:99999,marginTop:-60,}}>
                    <Text style={[styles.sheading,{fontSize:40}]}>1</Text>
                    {/* <Image source={require('../assets/botz/king.png')} resizeMode={'contain'} style={{width:35,height:35,alignSelf:'center',marginVertical:5}}/> */}
                    <ImageBackground source={require('../assets/botz/no1.png')} resizeMode='stretch' style={{width:110,alignItems: 'center',height:190,paddingTop:35}}>
                      <Image source={require('../assets/logom.png')} resizeMode={'contain'} style={{width:80,height:80}} />
                    </ImageBackground>
                    <Text style={[styles.text,{textAlign: 'center',color:'#fff',fontWeight: 'bold',fontSize:20}]}>{Data[0].name}</Text>
                    
                </View>
                <View style={{flexDirection:'column',justifyContent:'center',alignSelf: 'center',marginTop:10}}>
                <Text style={[styles.sheading,{fontSize:40}]}>3</Text>
                    {/* <Image source={require('../assets/botz/arrw2.png')} resizeMode={'contain'} style={{alignSelf:'center',marginVertical:5}}/> */}
                    <ImageBackground source={require('../assets/botz/no3.png')} resizeMode='stretch' style={{width:100,alignItems: 'center',height:160,paddingTop:45}}>
                      <Image source={require('../assets/logom.png')} resizeMode={'contain'} style={{width:60,height:60}} />
                    </ImageBackground>
                    <Text style={[styles.text,{textAlign:'center',color:'#fff',fontWeight: 'bold',fontSize:15}]} >{Data[2].name&&Data[2].name.split(' ')[0]}</Text>                    
                </View>
            </View>
        </View>
        </ImageBackground>
          ...
          renderItem={({item, index}) => (
            ...
            <View>
              <View  style={{marginVertical:3,width:'100%',alignSelf: 'center',borderRadius:5,flexDirection:'row',alignItems: 'center',paddingVertical:5,justifyContent: 'space-between',borderBottomWidth:0.5,borderBottomColor:'grey'}}>                                                
                 <View style={{flexDirection:'row',justifyContent:'space-between',alignItems: 'center',alignSelf: 'flex-start',marginLeft:15,width:"60%"}}>
                  <View>
                  ...
                  <Text style={{fontWeight:'bold',fontSize:15,width:150,color:colors.selected}}>{item.name}</Text>                   

                  </View>
                  </View>
                  <View  style={{felx:1,backgroundColor:'#4a9529',borderRadius:50,width:90,padding:5,paddingVertical:7,marginRight:15,alignItems: 'center',flexDirection:'row',justifyContent: 'space-evenly'}}>
