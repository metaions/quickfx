...
var bg='#171E26'
var light='#1F2630'
const TransferScreenPool = ({ navigation,route }) => {
  const botList=[
    // {label: 'MetaFx', value: 'metafx'},
    {label: 'Income', value: 'Income'},
    {label: 'PoolProfit', value: 'PoolProfit'},
    // {label: 'MetaFuture', value: 'metafuture'},
   
  ];
  const [selectedBot,setSelectedBot] = React.useState('Income')
  ...
  const txnPasswordSubmit=()=>{
    if(parseFloat(Verify)!==parseFloat(OTP)){
      ...
      if(global.txnPassword===txnPassword)
      {
          ...
          console.log('going for withdraw');
            Withdraw()
      }
      else{
        setModalVisible(false)
...
// +'&toid='+Uid
    var url =global.BASE_URL+'css_mob/transfer_pool.aspx?uid='+Uid+'&amt='+Amt+'&from='+from+'&pwd='+Pwd+'&'+key_string+'&'+ePass+'&token='+Token+'&device='+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
   ...
   try{
    if(dta.success==='false' && dta.msg==='wrong_pwd'){
      ...
      setClicked(false)
      ToastAndroid.show('Please try later!',ToastAndroid.SHORT)
      navigation.goBack()
    })
  }
  const toggleModal = () => {
  ...
  const getBal =(uid,type)=>{
    var url =global.BASE_URL+'css_mob/bal.aspx?uid='+uid+'&ttype='+type
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
  ...
  return (
    ...
    <View style={[styles.container]}>
          ...
          <View style={{flexDirection:'column',alignItems: 'center',justifyContent:'center',width:'15%',paddingTop:5}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between',width:'80%',paddingLeft:0}}>            
                    <Text style={[styles1.heading,{color:colors.selected,fontSize:16}]}>Transfer to Pool Wallet</Text>                            
        </View>
  ...
  <View style={styles1.action} >
               <Text style={{color:colors.binanceylw2,marginBottom:5}}>Transfer Amount</Text>     
                    ...
                    />
                     <Text style={{color:colors.binanceylw2,marginVertical:5,marginTop:10}}>From Wallet</Text>     
                     <Dropdown
                                  style={[styles.dropdown, { borderColor: 'white',borderWidth:0 ,width: '100%',backgroundColor:colors.c3, 
                                  paddingLeft:25,
                                  paddingVertical:20,}]}
                                  ...
                                  data={botList}
                                  activeColor={'#d3d3d3'}
                                  containerStyle={{marginTop:-40,borderRadius:10,}}
               ...
               <View style={{width:'100%',backgroundColor:colors.c3,marginTop:20,alignItems: 'center',}}>
                ...
                <Text style={styles1.transColor}>4 USDT</Text>
  </View>
                        ...
                        flexDirection:'row',marginTop:10,paddingHorizontal:10,paddingVertical:10,justifyContent: 'space-between'}}>
                            ...
                            <Text style={styles1.transColor}>{Amt!==''?(parseFloat(Amt)-4).toString()+' USDT' :''}</Text>
             ...
             <View style={styles.button}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => { if(Amt!==''){setLd(true),toggleModalOtp()}
...
export default TransferScreenPool;
