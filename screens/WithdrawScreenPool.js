...
var bg='#171E26'
var light='#1F2630'
const WithdrawScreenPool = ({ navigation,route }) => {
  ...
  const getBal =(uid)=>{
    var url =global.BASE_URL+'css_mob/bal.aspx?uid='+uid+'&ttype=P'
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
  ...
  const Withdraw =()=>{
    ...
    const ePass = objToQueryString({
       epass:txnPassword      
          ...
          var url =global.BASE_URL+'css_mob/ww_pool.aspx?uid='+Uid+'&amt='+Amt+'&addr='+Addr+'&pwd='+Pwd+'&otp='+Verify+'&api_key='+global.api_key+'&api_secret='+global.api_secret +'&'+key_string+'&'+ePass+'&token='+Token+'&device='+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()
          console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
...
const history=(uid)=>{
    let url= global.BASE_URL+'css_mob/history.aspx?uid='+uid+'&ttype=P&dsc1=Withdrawal';    
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
...
const[fee,setFee] = React.useState('')
  ...
  {/* </View> */}
                      <TouchableOpacity onPress={Count?null:() =>{setCount(true),OTPCall()}} style={{alignSelf: 'center',marginLeft:10}} >
                              ...
                              height:40,paddingHorizontal:5,borderRadius:5,paddingVertical:5,width:110,backgroundColor:"#065C91"}]}>                               
...
export default WithdrawScreenPool;
