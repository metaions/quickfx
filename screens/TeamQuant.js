...
const TeamQuant = ({ navigation }) => {
    ...
    const callApi=(uid)=>{
        let url =global.BASE_URL +"css_mob/rewards.aspx?uid="+uid+"&type=3"
        console.log(url)
        fetch(url)
        .then(item=> item.json())
        .then(dta=>{
    ...
    const rev_det=(date)=>{
        setLoading(true)
        let url=global.BASE_URL +"css_mob/rewards.aspx?uid="+Uid+"&dtday="+date+"&type=3";
        console.log(url)
        fetch(url)
        .then(item=>item.json())
        .then(dta=>{
