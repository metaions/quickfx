...
import { View, Text, Button, Dimensions,RefreshControl, 
  TouchableOpacity,ToastAndroid,Clipboard, StyleSheet,
   Image, StatusBar, FlatList, ScrollView, TextInput,ActivityIndicator, ImageBackground} from 'react-native';
...
//   setLoading(true),Withdraw()
  const Withdraw =()=>{
    function objToQueryString(obj) {
      ...
      const key_string = objToQueryString({
         key:Uid+Old_pwd+global.txnPassword      
    });  
      
      const ePass = objToQueryString({
         epass:global.txnPassword      
    });  
    var url =global.BASE_URL+'css_mob/change.aspx?uid='+Uid+'&oldpwd='+Old_pwd+'&newpwd='+New_pwd+'&'+key_string+'&'+ePass+'&token='+token+'&device='+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()
    console.log(url)
    try{
  ...
  return (
    ...
    <ImageBackground source={global.bgimg} resizeMode='stretch' style={[styles.container,]}>
          <Modal onBackButtonPress={()=>{setOtpModal(false)}} onBackdropPress={()=>{setOtpModal(false)}} useNativeDriver={true}
            animationIn={'slideInUp'} animationOut={'slideOutDown'}
     isVisible={otpModal}  style={{alignSelf:'center'}} animationInTiming={300} animationOutTiming={200}
    >
  ...
  {/* <View style={{width:350,alignItems: 'flex-start',alignSelf: 'center',backgroundColor:'transparent',
      ...
      >
          ...
          <Text style={{color:'black',fontSize:16,fontFamily:global.appFontM}}>Withdraw</Text>
          ...
          <View style={{flexDirection:'row',width:'98%',paddingLeft:10}}>            
              <TouchableOpacity onPress={()=>navigation.goBack()} style={{padding:10,marginTop:2,marginRight:10}}>
                  <Text style={{textAlign:'right'}}><IonIcons name="md-arrow-back" size={22}  color={colors.selected}   /></Text>
                      ...
                      <Text style={[styles.heading,{color:colors.profitcolor2,textTransform:'uppercase',fontSize:21}]}>Change Password</Text>                            
          </View>
     ...
     backgroundColor:colors.background}} ><LottieView source={require('../assets/loading.json')} 
     style={{width:150,height:100,alignSelf:'center'}} autoPlay loop /></View>
      :
      <View style={{marginBottom:'30%',position:'relative',top:10,alignSelf:'center',width:'100%',
      ...
      {/* </ImageBackground> */}
             ...
             <TouchableOpacity 
                    ...
                    padding:5,borderRadius:30,height:50,alignItems: 'center',justifyContent: 'center',alignSelf: 'center',width:'80%'}]}>
