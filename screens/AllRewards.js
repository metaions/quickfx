...
import { View, Text, Button, Dimensions, Switch, ImageBackground, TouchableOpacity, Linking, StyleSheet, RefreshControl, Image, StatusBar, FlatList, ScrollView, TextInput, ActivityIndicator, ToastAndroid } from 'react-native';
...
const AllRewards = ({ navigation }) => {
    ...
    const callApi = (uid) => {
        let url = global.BASE_URL + "css_mob/rewards.aspx?uid=" + uid + "&type=All"
        console.log(url)
        fetch(url)
            .then(item => item.json())
            .then(dta => {
    ...
    const rev_det = (date) => {
        setLoading(true)
        let url = global.BASE_URL + "css_mob/rewards.aspx?uid=" + Uid + "&dtday=" + date + "&type=all";
        console.log(url)
        fetch(url)
            .then(item => item.json())
            .then(dta => {
    ...
    return (
        ...
        <View style={{ flexDirection: 'column', justifyContent: 'center', width: '100%' }}>
                        ...
                        >
                            <View style={{
                                ...
                                resizeMode={'stretch'} style={{
                                    ...
                                    <Text style={[{ textAlign: 'center', fontSize: 12,textTransform:'uppercase', color: colors.selected,  
                                    ...
                                    <Text style={[{ textAlign: 'left', fontSize: 13, color: colors.selected, marginTop:48,fontFamily:global.appFontM }]}>{(parseFloat(TP) * global.cur_value).toFixed(2)} {global.cur_name}</Text>
                                ...
                                resizeMode={'stretch'} style={{
                                    ...
                                    <Text style={[{ textAlign: 'left', fontSize: 13, color: colors.selected, marginTop:48,fontFamily:global.appFontM }]}>{(parseFloat(CP) * global.cur_value).toFixed(2)} {global.cur_name}</Text>
                    ...
                    <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center',bottom:65,right:2}}>
                        <Image source={require('../assets/Aeon/logoCircle.png')} resizeMode="stretch" style={{width:50,height:51,tintColor:'transparent'}} />
                                    ...
                                    flexDirection: 'row',width:'50%',alignItems:'center',
                                     alignSelf: 'flex-start',justifyContent:'center'
                                        ...
                                        <View style={{
                                            ...
                                            <Text style={{ fontSize: 12,fontFamily:global.appFontM,  color: '#fff' }}>PROFIT</Text>
                                            
                                            <Text style={{ color:colors.profitcolor2, fontSize: 14,fontFamily:global.appFontB }}>${parseFloat(item.amount).toFixed(3)}</Text>
                                        ...
                                        <View style={{
                                            ...
                                            <Text style={{ fontSize: 12,  fontFamily:global.appFontM, color: '#fff' }}>DATE</Text>
                                           <Text style={{ color: '#8E99BA', fontSize: 14,fontFamily:global.appFontB, marginBottom: 5, }}>{item.date}</Text>
