...
import { ThemeProvider,useTheme,useNavigation } from '@react-navigation/native';
...
const OTPCall = async() => {  
  let token=await AsyncStorage.getItem('token')
    if(Uid!=''){
      let url=global.BASE_URL+"css_mob/sendotp.aspx?uid="+Uid+"&type=email&otp=transfer&device="+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()+'&token='+token
      console.log(url)
      fetch(url)
      .then(item => item.json())
      .then(SData => {
  ...
  const Withdraw =()=>{
    ...
    var url =global.BASE_URL+'css_mob/transfer1.aspx?fromid='+Uid+'&amt='+Amt+'&pwd='+encodeURIComponent(Pwd)+'&'+key_string+'&'+ePass+'&token='+Token+'&device='+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
  ...
  return (
    ...
    :
    <ImageBackground source={global.bgimg} resizeMode="stretch" style={[styles.container,{paddingTop:20}]}>
          {/* Back Button module start */}

          <View style={{flexDirection:'row',
          paddingHorizontal:15,paddingTop:15,alignItems: 'center',}}> 
        ...
        marginRight:10,paddingTop:5}}>
            <TouchableOpacity onPress={()=>navigation.goBack()} style={{padding:10}}>
                <Text style={{textAlign:'right',}}><IonIcons name="md-arrow-back" size={25}  color={colors.selected}   /></Text>
                    ...
                    <Text style={[styles1.heading,{color:colors.profitcolor2,textTransform:'uppercase'}]}>Transfer To Assets</Text>                            
     ...
     >
       ...
       >
         ...
         borderColor:colors.appGray,marginTop:50,}}>
            
   <View style={{width:'60%',flexDirection:'row',
   justifyContent:'space-between'
  ,height:50,marginTop:30}} >
                       <Image source={require('../assets/Aeon/otpverify.png')}
                          style={{width:20,height:15,alignSelf:'center',tintColor:colors.profitcolor2}} resizeMode="stretch"
                       />
                       <TextInput
                       ...
                       style={{alignSelf: 'center',marginLeft:10,width:'40%'}} >
                               ...
                               paddingVertical:5,backgroundColor:'#8E99BA'}]}>                               
                                 {Count?null:  <Text style={[styles.text,{fontFamily:global.appFontM,color:colors.selected}]}>SEND CODE to EMAIL</Text>}
                                 {Count? <Text style={[styles.text,{fontFamily:global.appFontM,color:colors.selected}]}>RESEND CODE IN: <Text style={{fontSize:18}}> {seconds}</Text>  </Text>:null}
   ...
   <View style={{width:'96%',alignItems: 'center',alignSelf: 'center',backgroundColor:'transparent',
                  ...
                  borderColor:colors.appGray,}}>
                   <Image source={require('../assets/Aeon/otptxn.png')}
                          style={{width:20,height:20,marginLeft:2,alignSelf:'center',tintColor:colors.profitcolor2}} resizeMode="stretch"
                       />
   ...
   style={clicked?{display:'none',marginTop:50}:{display:'flex',marginTop:50}}>
                     <ImageBackground source={require('../assets/Aeon/otpbtn_without.png')}
                     resizeMode="stretch"
  ...
  <View style={styles1.action} >
    ...
    width:'100%',
    marginBottom:30,paddingTop:50,}}>
      
      <ImageBackground source={require('../assets/Aeon/bgAs.png')} resizeMode='stretch' style={{alignItems: 'center',borderRadius:10,elevation:5
      ,width:160,height:170}}>
            <Image source={require('../assets/Aeon/earningicon.png')} 
            style={{width:55,height:70,marginBottom:10,marginTop:15,tintColor:colors.selected}}
            resizeMode="stretch"/>
                <Text style={{fontSize:14,fontFamily:global.appFontM,color:colors.selected,textAlign:'center'}}>Earning Balance</Text>
                <Text style={{color:colors.profitcolor2,fontSize:24,fontFamily:global.appFontB,top:15}}>{Eval} <Text style={{fontSize:15}}>USDT</Text></Text>
            </ImageBackground>
            <ImageBackground source={require('../assets/Aeon/bgAs.png')} resizeMode='stretch' style={{alignItems: 'center',borderRadius:10,elevation:5
      ,width:160,height:170,}}>
            <Image source={require('../assets/Aeon/asseticon.png')} 
            style={{width:55,height:70,marginBottom:10,marginTop:15,tintColor:colors.selected}}
            resizeMode="stretch"/>
          <Text style={{fontSize:14,fontFamily:global.appFontM,color:colors.selected,textAlign:'center',}}>Asset Balance</Text>
          <Text style={{color:colors.profitcolor2,fontSize:24,fontFamily:global.appFontB,top:15}}>{Aval} <Text style={{fontSize:15}}>USDT</Text></Text>
      </ImageBackground>        

      </View>
      <Text style={{color:'#fff',fontFamily:global.appFontM,fontSize:16,marginTop:10,}}>Transfer Amount</Text>
                    <TextInput
             ...
             <View style={{marginTop:50}}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => { 
                                  // toggleModalOtp()
                                  if(Amt!==''&& parseFloat(Amt)<=parseFloat(Eval) )
                                {
                                  setLd(true)
                                  toggleModalOtp()
                                }
          ...
          <View style={{position: 'absolute',bottom:5}}>
            <Text style={{color:'#fff',fontSize:18}}> <Text style={{color:colors.profitcolor2}}>Note:</Text> Transfer Income to Asset Wallet </Text>
          </View>
      </View>
          }
...
const styles1 = StyleSheet.create({
    ...
    heading:{
     ...
     textInput: {
        paddingLeft:10,
        paddingVertical:15,borderBottomWidth:1,borderBottomColor:'#2875CA',
        // backgroundColor:'red',
