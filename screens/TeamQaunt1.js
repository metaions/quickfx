...
import { View, Text,ImageBackground, Button, Dimensions,Switch, TouchableOpacity,Linking, StyleSheet,RefreshControl, Image, StatusBar, FlatList, ScrollView, TextInput,ActivityIndicator, ToastAndroid } from 'react-native';
...
const TeamQuant1 = ({ navigation }) => {
    ...
    const callApi=(uid)=>{
        let url =global.BASE_URL +"css_mob/rewards.aspx?uid="+uid+"&type=4"
        console.log(url)
        fetch(url)
        .then(item=> item.json())
        .then(dta=>{
    ...
    const rev_det=(date)=>{
        setLoading(true)
        let url=global.BASE_URL +"css_mob/rewards.aspx?uid="+Uid+"&dtday="+date+"&type=4";
        console.log(url)
        fetch(url)
        .then(item=>item.json())
        .then(dta=>{
        ...
        <View style={{ flexDirection: 'column', justifyContent: 'center', width: '100%' }}>
                        ...
                        >
                            <View style={{
                                ...
                                <ImageBackground source={require('../assets/Aeon/box1s.png')}
                                // imageStyle={{width:'50%'}}
