...
import { View, Text, Button, Dimensions, Switch, 
    ImageBackground, TouchableOpacity, Linking, StyleSheet, 
    RefreshControl, Image, StatusBar, FlatList, ScrollView, TextInput, 
...
const Profit_Income = ({ navigation }) => {
    ...
    const callApi = () => {
        let url = global.BASE_URL + "css_mob/rewards_all.aspx?uid=" + Uid+'&mode='+status;
        console.log(url)
        fetch(url)
            .then(item => item.json())
            .then(dta => {
    ...
    const rev_det = (date) => {
        setLoading(true)
        let url = global.BASE_URL + "css_mob/rewards.aspx?uid=" + Uid + "&dtday=" + date + "&type=1";
