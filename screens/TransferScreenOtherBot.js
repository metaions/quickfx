...
var bg='#171E26'
var light='#1F2630'
const TransferScreenOtherBot = ({ navigation,route }) => {
  const botList=[
    // {label: 'MetaFx', value: 'metafx'},
    {label: 'MetaGames', value: 'metagames'},
    {label: 'BotZ', value: 'botz'},
    {label: 'MetaFuture', value: 'metafuture'},
   
  ];
  const [selectedBot,setSelectedBot] = React.useState('metafuture')
...
const OTPCall = async() => {  
  ...
  const Withdraw =()=>{
    ...
    var url =global.BASE_URL+'css_mob/transfer_bot.aspx?fromid='+Uid+'&amt='+Amt+'&toid='+Uid+'&app_name='+selectedBot+'&pwd='+Pwd+'&'+key_string+'&'+ePass+'&token='+Token+'&device='+DeviceInfo.getUniqueId()+'&dname='+DeviceInfo.getModel()
    console.log(url)
    fetch(url)
    .then(item=>item.json())
    .then(dta=>{
  ...
  <View style={styles1.action} >
               <Text style={{color:colors.binanceylw2,marginBottom:5}}>Transfer Amount</Text>     
                    ...
                    />
                     <Text style={{color:colors.binanceylw2,marginVertical:5,marginTop:10}}>Bot</Text>     
                     <Dropdown
                                  ...
                                  data={botList}
                                  activeColor={'grey'}
                                  containerStyle={{marginTop:-40,borderRadius:10,backgroundColor:'#bbb'}}
               ...
               <View style={{width:'100%',backgroundColor:colors.c3,marginTop:20,alignItems: 'center',}}>
                ...
                <Text style={styles1.transColor}>1 USDT</Text>
  </View>
                        ...
                        flexDirection:'row',marginTop:10,paddingHorizontal:10,paddingVertical:10,justifyContent: 'space-between'}}>
                            ...
                            <Text style={styles1.transColor}>{Amt!==''?(parseFloat(Amt)-1).toString()+' USDT' :''}</Text>
...
export default TransferScreenOtherBot;
