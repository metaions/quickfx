...
export default function WebChart({sym,navigation}) {
    
    const {colors} = useTheme();
    // const sym = route.params?.sym
    const [url, setUrl] = useState('https://ios.botz.trade/chart_web?sym='+sym);
    ...
    useEffect(() => {
        return;
        console.log("nav in webview1" + 'https://ios.botz.trade/chart_web?sym='+sym);

        setUrl('https://ios.botz.trade/chart_web?sym='+sym);
        // setTimeout(()=>{
        //         setLd(false)
        // },1500)
    }, [isFocused])
